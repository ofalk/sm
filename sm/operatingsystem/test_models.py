...
class Tester(TestCase):
    ...
    def test_version(self):
        self.assertEqual(self.testitem.version, self.teststring, "version not correct")

    def test___str__(self):
        self.assertEqual(
            "{} {}".format(self.vendor.name, self.teststring),
            "{} {}".format(self.testitem.vendor.name, self.testitem.version),
    ...
    def test_natural_key__(self):
        self.assertEqual(
            self.testitem.natural_key(), (self.vendor.name, self.teststring)
        )
    ...
    def test_get_rhel_7_by_nat_key(self):
        ...
        self.assertIsInstance(item, self.model, "object not correct model!?")
    ...
    def test_get_sles10_nat_key(self):
        ...
        item, created = self.model.objects.get_or_create(vendor=vendor, version="10.0")
        self.assertIsInstance(item, self.model, "object not a Operatingsystem model!?")
    ...
    def test_nat_key_funny_doesnt_exist(self):
        ...
        self.assertTrue("matching query does not exist" in str(context.exception))

    def test_nat_key_vendor_version_exists(self):
        item = self.model.objects.get_by_natural_key(
            vendor=self.vendor.name, version=self.teststring
        )
        self.assertIsInstance(item, self.model, "object not correct model!?")
    ...
    def test_query_with_dict_exception(self):
        with self.assertRaises(Exception) as context:
            self.model.objects.get_by_natural_key({})
        self.assertTrue("No idea how to handle query with " in str(context.exception))
...
class GlobalUniquenessTest(TestCase):
    ...
    def _make(self, version, group):
        return self.model.objects.create(
            version=version, vendor=self.vendor, group=group
        )

    def test_duplicate_global_row_rejected(self):
        version = "v%s" % random_string()
        self._make(version, None)
    ...
    def test_same_identity_allowed_across_groups(self):
        version = "v%s" % random_string()
        a = self._make(version, self.group_a)
        b = self._make(version, self.group_b)
