...
class Tester(TestCase):
    ...
    def setUp(self):
        ...
        self.cluster = ClusterModel.objects.all().order_by("name").first()
        self.patchtime = PatchtimeModel.objects.all().order_by("name").first()
        self.location = LocationModel.objects.all().order_by("name").first()
        self.servermodel = ServermodelModel.objects.all().order_by("name").first()
        self.domain = DomainModel.objects.all().order_by("name").first()
        self.status = (
            StatusModel.objects.exclude(name="Disposed").order_by("name").first()
        )
    ...
    def test_listview(self):
        ...
        self.assertEqual(item.cluster.pk, self.cluster.pk)
    ...
    def test_deleteview(self):
        ...
        self.assertEqual(response.status_code, 200, "no status 200?")
        item = response.context[-1]["object"]
        self.assertIsInstance(item, Model, "oobject not the correct model!?")
        self.assertEqual(item.hostname, self.teststring)
    ...
    def test_deleteview_post(self):
        ...
        pass
        self.assertContains(
            response, "%s was deleted successfully" % self.testitem.hostname
        )
    ...
    def test_createview(self):
        ...
        self.assertRaises(
            KeyError, form.fields["hostname"].widget.attrs.__getitem__, "disabled"
        )
