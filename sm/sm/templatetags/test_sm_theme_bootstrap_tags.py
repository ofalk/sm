...
class TestCase(TestCase):
    ...
    def test_01_warning(self):
        ...
        rendered = Template(string).render(
            Context(
                {
                    "message": TestCase.FakeMessage(
                        DEFAULT_MESSAGE_LEVELS.WARNING, "warning message"
                    )
                }
            )
        )
    ...
    def test_02_error(self):
        ...
        rendered = Template(string).render(
            Context(
                {
                    "message": TestCase.FakeMessage(
                        DEFAULT_MESSAGE_LEVELS.ERROR, "error message"
                    )
                }
            )
        )
    ...
    def test_02_warning_with_extra_tag(self):
        ...
        rendered = Template(string).render(
            Context(
                {
                    "message": TestCase.FakeMessage(
                        DEFAULT_MESSAGE_LEVELS.WARNING, "message", "testtag"
                    )
                }
            )
        )
    ...
    def test_03_extra_tag_only(self):
        ...
        string += "{% get_message_tags message %}"
        rendered = Template(string).render(
            Context({"message": TestCase.FakeMessage(None, "message", "testtag")})
        )
    ...
    def test_04_empty(self):
        ...
        string += "{% get_message_tags message %}"
        rendered = Template(string).render(
            Context({"message": TestCase.FakeMessage(None, "")})
        )
        self.assertEqual(rendered, "")
