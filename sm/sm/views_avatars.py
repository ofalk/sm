...
def avatar_proxy(request, email_hash):
    ...
    if cached_avatar:
        response = HttpResponse(cached_avatar['content'], content_type=cached_avatar['content_type'])
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            cache.set(cache_key, {'content': res.content, 'content_type': content_type}, 86400)
            
            response = HttpResponse(res.content, content_type=content_type)
            response['Cache-Control'] = 'public, max-age=604800, immutable' # 1 week browser cache
            return response
    except Exception:
        pass
